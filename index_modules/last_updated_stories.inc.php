<table width="280" border="0" cellpadding="0" cellspacing="0" class="topstories" style="background-image: url(<?=IMAGE_HOST?>/site_gfx_new/bg_book.png);">
  <tr>
    <td colspan="2" valign="top"><img src="<?=IMAGE_HOST?>/site_gfx_new/DD_storiesUpdate.png" width="280" height="25" class="headerimg" /></td>
  </tr>
  <tr>
    <td colspan="2" class="padding" style="background-image: url(<?=IMAGE_HOST?>/site_gfx_new/bg_grad.png);">
      <?
      $res = db_query("SELECT comic_name, last_update, category, flags FROM comics WHERE comic_type='1' AND total_pages>0 AND delisted=0 ORDER BY last_update DESC LIMIT 5");
