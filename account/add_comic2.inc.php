<?
  ...
  if ( !isset($_POST['newComicCat']) || !isset($COMIC_CATS[$_POST['newComicCat']]) ) {
    echo("<SPAN CLASS='alert'>There was an error in your request. <A HREF='JavaScript:history.back();'>Click here to go back and try again.</A></SPAN>");
    return;
  }
  ...
  db_query("INSERT INTO comics (user_id, comic_name, comic_type, category, subcategory, description, created_timestamp) VALUES ('".$USER->user_id."', '".$comic_name."', '".$comic_type."', '".$comic_cat."', '".$comic_subcat."', '".$description."', '".time()."')");
