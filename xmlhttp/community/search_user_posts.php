...
$res = db_query("SELECT * FROM users WHERE username='".db_escape_string($search)."'");
...
$res = db_query("SELECT * FROM community_posts WHERE user_id='".$uRow->user_id."' ORDER BY date_created DESC LIMIT 25");
...
foreach($POST_ROWS as $row)
{
  ...
  if ( ($CAT_ROWS[$TOPIC_ROWS[$row->topic_id]->category_id]->flags & FORUM_FLAG_ADMIN_ONLY)  )
  {
    ...
    {
      ?><li id="<?=$row->topic_id?>"><?=htmlentities($TOPIC_ROWS[$row->topic_id]->topic_name, ENT_QUOTES)?><span class="informal"><?=nl2br(bbcode( trim(substr($row->post_body, 0, 100)).'...'))?><span class="informal_rt">Created: <?=date("n.d.Y", $row->date_created)?></span></span></li><?
      if ( ++$ct%5 == 0 ) break;
    }
