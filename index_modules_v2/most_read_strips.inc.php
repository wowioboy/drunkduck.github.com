...
function showMoreMostReadStrips()
{
  for(var i=6; i<11; i++) {
    $('strip_'+i).style.display = '';
  }
}
</script>
<img src="<?=IMAGE_HOST_SITE_GFX?>/sect_strips1.gif" width="212" height="30" />
<div class="narrow">
  <table width="206" border="0" cellpadding="0" cellspacing="0" class="topstories">
    <tr>
      <td width="80" rowspan="10" align="center" valign="top" id='top_strips_prev'>
        <?
        $extra = '';
        ...
        $res = db_query("SELECT comic_id, comic_name, last_update, category, rating_symbol, flags FROM comics WHERE comic_type='0' ".$extra." AND total_pages>0 AND delisted=0 ORDER BY seven_day_visits DESC LIMIT 10");
      ...
      while ($row = db_fetch_object($res))
      {
        ...
        $id = '';
        if ( $ct > 5 ) {
          $id = 'id="strip_'.$ct.'" style="display:none;"';
        }
        ?><td <?=$id?> width="116" height="20" valign="middle" class="padding" onMouseOver="$('top_strips_prev').innerHTML='<?=str_replace("'", "\\'", "<img src='".thumb_processor($row)."'>")?>';">
    ...
    <tr>
      <td height="20" colspan="2" align="right" valign="middle" id='top_stories_prev'>
        <a href="#" onClick="showMoreMostReadStrips();this.style.display='none';return false;"><img src="<?=IMAGE_HOST_SITE_GFX?>/smbtns_more.gif" width="56" height="14" border="0" /></a>
        <a href="http://<?=DOMAIN?>/search.php?browsetype=0&sortby=read"><img src="<?=IMAGE_HOST_SITE_GFX?>/smbtns_browse.gif" width="56" height="14" border="0" /></a>
      </td>
    </tr>
  </table>
</div>